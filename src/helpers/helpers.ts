...
export const getFirstLetter = (text: string, letterCount = 2): string =>
	...
	text
		...
		export const getFirstttLetter = (text: string, letterCount = 2): string => {
			if (!text) return '';  // Handle cases where 'text' is undefined, null, or empty string
		  
			// Ensure the text is a string and prevent errors from undefined or null values
			return text
			  .toString()  // Converts the value to string if it's not already
			  .toUpperCase()
			  .match(/\b(\w)/g)
			  ?.join('')  // Safely join the letters if match is found
			  .substring(0, letterCount) || '';  // Default to empty string if there's an issue
		  };
